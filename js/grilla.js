...
Tablero.prototype.setColum =  function (col){
    for (var i = 0; i < this.Matrix[col].length; i++) {
    //pinto una columna
    this.Matrix[col][i] = 1;
  }
}
...
Tablero.prototype.queColumnaAmeo = function (x,y) {
  ...
  for (var col = 0; col < columnas; col++) {
        if (((col*celda)+offsetX)<= x && ((col+1)*celda)+ offsetX >= x){
            return col;
        }
  }
  return -1;
    ...
    var offsetY = (1/3)*ctx.canvas.height-10;//se le resta 10 para que los circulos queden separados del borde
      for (var x = 0; x <= columnas; x++) {
          for (var y = 0; y <= filas; y++) {
             // ctx.moveTo((x * celda) + offsetX, offsetY);
             // ctx.lineTo((x * celda) + offsetX, (filas * celda) + offsetY);
             // ctx.stroke();
             // ctx.moveTo(offsetX, (y * celda) + offsetY);
             // ctx.lineTo((columnas * celda) + offsetX, (y * celda) + offsetY);
             // ctx.stroke();
